...
class DataAugmentor_Video(object):
    def __init__(self, root_path, augmentor_configs, class_names, seq_length, logger=None):
        self.root_path = root_path
        self.class_names = class_names
        self.seq_length = seq_length
        self.logger = logger
        self.is_seq_db_sampling = augmentor_configs.IS_SEQ_DB_SAMPLING
        self.data_augmentor_queue = []
        aug_config_list = augmentor_configs if isinstance(augmentor_configs, list) \
            else augmentor_configs.AUG_CONFIG_LIST
    ...
    def gt_sampling(self, config=None):
        if self.is_seq_db_sampling:
            db_sampler = database_sampler.DataBaseSampler_Sequential(
                root_path=self.root_path,
                sampler_cfg=config,
                class_names=self.class_names,
                logger=self.logger
            ...
            db_sampler = database_sampler.DataBaseSampler_Non_Sequential(
                root_path=self.root_path,
                sampler_cfg=config,
                class_names=self.class_names,
                logger=self.logger
    ...
    def random_world_flip(self, data_dict_seq_list=None, config=None):
        
        if data_dict_seq_list is None:
            return partial(self.random_world_flip, config=config)
        ...
        for data_dict in data_dict_seq_list:
            gt_boxes, points = data_dict['gt_boxes'], data_dict['points']
            gt_boxes_seq_list.append(gt_boxes)
            points_seq_list.append(points)

        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y']
            gt_boxes_seq_list, points_seq_list = getattr(augmentor_utils, 'random_flip_along_%s_video' % cur_axis)(
                gt_boxes_seq_list, points_seq_list,
            )

        for i , data_dict in enumerate(data_dict_seq_list):
            data_dict['gt_boxes'] = gt_boxes_seq_list[i]
            data_dict['points'] = points_seq_list[i]
        
        return data_dict_seq_list

    def random_world_rotation(self, data_dict_seq_list=None, config=None):
        if data_dict_seq_list is None:
            return partial(self.random_world_rotation, config=config)
        rot_range = config['WORLD_ROT_ANGLE']
        if not isinstance(rot_range, list):
        ...
        gt_boxes_seq_list, points_seq_list = augmentor_utils.global_rotation_video(
            gt_boxes_seq_list, points_seq_list, rot_range=rot_range
        )
    ...
    def random_world_scaling(self, data_dict_seq_list=None, config=None):
        if data_dict_seq_list is None:
            return partial(self.random_world_scaling, config=config)
        ...
        gt_boxes_seq_list, points_seq_list = augmentor_utils.global_scaling_video(
            gt_boxes_seq_list, points_seq_list, config['WORLD_SCALE_RANGE']
        )
    ...
    def random_world_translation(self, data_dict_seq_list=None, config=None):
        if data_dict_seq_list is None:
            return partial(self.random_world_translation, config=config)
        noise_translate_std = config['NOISE_TRANSLATE_STD']
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y', 'z']
            gt_boxes, points = getattr(augmentor_utils, 'random_translation_along_%s_video' % cur_axis)(
                gt_boxes_seq_list, points_seq_list, noise_translate_std,
            )
        for i , data_dict in enumerate(data_dict_seq_list):
            data_dict['gt_boxes'] = gt_boxes_seq_list[i]
            data_dict['points'] = points_seq_list[i]
    ...
    def forward(self, data_dict_seq_list):
        ...
        Args:
            data_dict_seq_list:
                len(data_dict_seq_list) = self.seq_length
                type(data_dict_seq_list[i]) = Dictionary
                data_dict_seq_list[i].keys() = dict_keys(['points', 'frame_id', 'metadata', 'gt_names', 'gt_boxes', 'test_key', 'gt_boxes_mask'])
                points: (N, 3 + C_in)
                gt_boxes: optional, (N, 7) [x, y, z, dx, dy, dz, heading]
                gt_names: optional, (N), string
                ...
        ...
        """
        for cur_augmentor in self.data_augmentor_queue:
            data_dict_seq_list = cur_augmentor(data_dict_seq_list=data_dict_seq_list)
