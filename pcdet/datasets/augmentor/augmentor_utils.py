...
def random_flip_along_x_video(gt_boxes_seq_list, points_seq_list):

    enable = np.random.choice([False, True], replace=False, p=[0.5, 0.5])
    if enable:

        for gt_boxes in gt_boxes_seq_list:
            gt_boxes[:, 1] = -gt_boxes[:, 1]
            gt_boxes[:, 6] = -gt_boxes[:, 6]
        ...
        for points in points_seq_list:
            points[:, 1] = -points[:, 1]

    return gt_boxes_seq_list, points_seq_list
...
def random_flip_along_y_video(gt_boxes_seq_list, points_seq_list):
...
def global_rotation_video(gt_boxes_seq_list, points_seq_list, rot_range):
    
    noise_rotation = np.random.uniform(rot_range[0], rot_range[1])
    
    for t in range(len(points_seq_list)):
        points_seq_list[t] = common_utils.rotate_points_along_z(points_seq_list[t][np.newaxis, :, :], np.array([noise_rotation]))[0]
...
def global_scaling_video(gt_boxes_seq_list, points_seq_list, scale_range):
    ...
    for i in range(len(gt_boxes_seq_list)):
        points_seq_list[i][:, :3] *= noise_scale
        gt_boxes_seq_list[i][:, :6] *= noise_scale
...
def random_translation_along_x_video(gt_boxes_seq_list:list, points_seq_list:list, offset_std):
...
def random_translation_along_y_video(gt_boxes_seq_list:list, points_seq_list:list, offset_std):
...
def random_translation_along_z_video(gt_boxes_seq_list:list, points_seq_list:list, offset_std):
